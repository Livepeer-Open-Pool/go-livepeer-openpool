...
docker:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t tztcloud/go-livepeer:v0.8.8-pool -f docker/Dockerfile .
## ** Pool Customization **
## Pool: Remote transcoders can run windows (NOT AI WORKERS!)
docker-windows:
	docker buildx build --build-arg='BUILD_TAGS=mainnet,experimental' -t go-livepeer:v0.8.8-pool.windows -f docker/Dockerfile.windows .

...
var schema = `
	...
	DROP VIEW IF EXISTS active_pool_workers_view;
	CREATE VIEW active_pool_workers_view AS
		WITH reset_time AS (
		    SELECT MAX(dt) AS last_reset_dt
		    FROM pool_events
		    WHERE json_extract(payload, '$.event_type') = 'orchestrator-reset'
		),
		latest_events AS (
		    SELECT
			json_extract(payload, '$.payload.ethAddress') AS ethAddress,
			json_extract(payload, '$.event_type') AS event_type,
			json_extract(payload, '$.payload.nodeType') AS nodeType,
			dt,
			ROW_NUMBER() OVER (
			    PARTITION BY json_extract(payload, '$.payload.ethAddress')
			    ORDER BY dt DESC
			) AS rn
		    FROM pool_events
		    WHERE
			json_extract(payload, '$.payload.ethAddress') IS NOT NULL
			AND dt > (SELECT last_reset_dt FROM reset_time)
		)
		SELECT
		    ethAddress,
		    nodeType,
		    CASE
			WHEN event_type = 'worker-connected' THEN 1
			ELSE 0
		    END AS is_connected
		FROM latest_events
		WHERE rn = 1;

	DROP VIEW IF EXISTS pool_jobs_view;
	CREATE VIEW pool_jobs_view AS
	SELECT
	    json_extract(payload, '$.payload.encodedPixels') AS encodedPixels,
	    json_extract(payload, '$.payload.ethAddress') AS ethAddress,
	    json_extract(payload, '$.payload.event_time') AS event_time,
	    json_extract(payload, '$.payload.fees') AS fees,
	    json_extract(payload, '$.payload.jobType') AS jobType,
	    json_extract(payload, '$.payload.price') AS price
	FROM pool_events
	WHERE json_extract(payload, '$.event_type') in ('transcode','ai');

	DROP VIEW IF EXISTS pool_performance_stats_view;
	CREATE VIEW pool_performance_stats_view AS
	SELECT
	    json_extract(payload, '$.payload.duration') AS duration,
	    json_extract(payload, '$.payload.ethAddress') AS ethAddress,
	    json_extract(payload, '$.payload.event_time') AS event_time,
	    json_extract(payload, '$.payload.pixelsDecoded') AS pixelsDecoded,
	    json_extract(payload, '$.payload.pixelsEncoded') AS pixelsEncoded,
	    json_extract(payload, '$.payload.jobType') AS jobType,
	    json_extract(payload, '$.payload.responseTime') AS responseTime,
	    json_extract(payload, '$.payload.sessionId') AS sessionId
	FROM pool_events
	WHERE json_extract(payload, '$.event_type') in ('performance-stats');
...
func InitDB(dbPath string) (*DB, error) {
	...
	// Find the active pool nodes
	stmt, err = db.Prepare("select ethAddress, nodeType, is_connected from active_pool_workers_view")
	if err != nil {
		glog.Error("Unable to prepare findActivePoolWorkers ", err)
		d.Close()
		return nil, err
	}
	d.findActivePoolWorkers = stmt

	// Find the pool job history
	stmt, err = db.Prepare("select ethAddress, encodedPixels, event_time,fees,jobType,price from pool_jobs_view")
	if err != nil {
		glog.Error("Unable to prepare findPoolJobHistory ", err)
		d.Close()
		return nil, err
	}
	d.findPoolJobHistory = stmt

	// Find the pool performance history
	stmt, err = db.Prepare("select ethAddress, duration, event_time, pixelsDecoded, pixelsEncoded, jobType, responseTime, sessionId from pool_performance_stats_view")
	if err != nil {
		glog.Error("Unable to prepare findPoolPerformanceHistory ", err)
		d.Close()
		return nil, err
	}
	d.findPoolPerformanceHistory = stmt
...
func (db *DB) FindJobHistoryEvents() ([]JobHistoryEvent, error) {
	rows, err := db.findPoolJobHistory.Query()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) FindPerformanceStatHistoryEvents() ([]PerformanceStatEvent, error) {
	rows, err := db.findPoolPerformanceHistory.Query()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
